...
class ImageType(FileType):
    ...
    def encode(
        ...
        image = Image.open(input_image_path).convert("RGB")  # Ensure RGB mode
    ...
    def encode_message_in_image(
        ...
        if len(final_message) > max_bytes_to_encode:
            raise ValueError(
                ...
                f"in image, but message length is {len(final_message)} bytes!"
            )
        ...
        # Repack flat list into RGB tuples
        pixel_size = len(input_image.getdata()[0])
        input_image.putdata(
            list(zip(*[iter(color_data)] * pixel_size))
        )
        return input_image
    ...
    def decode(
        ...
        steg_image = Image.open(encoded_image_path).convert("RGB")  # Ensure RGB mode
    ...
    def recover_message_from_image(
        ...
        if nr_lsb_used == 1 and USE_BYTE_SKIP:
            return FileType.decode_message_from_carrier_skip(
                color_data[file_size * 8:], 8 * bytes_to_recover, maximum_bytes_in_image
            )
        else:
            return FileType.decode_message_from_carrier(
                color_data[int(file_size * 8 / nr_lsb_used):], 8 * bytes_to_recover, nr_lsb_used
            )
